...
servers="$(curl -X GET --header 'Accept: application/json' --header "X-Auth-Token: '${PACKET_AUTH_TOKEN}'" "https://api.packet.net/projects/'${PACKET_PROJECT_ID}'/devices")"

#echo curl results
echo "************ servers in project ************"
echo $servers

#echo our current timestamp
echo "$(date -u)"
#Find if any creation timestamps > 4 hours

#Report them

#No Terraform setup needed
